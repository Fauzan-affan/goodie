...
class GamificationCreateUpdateSurvey extends Component {
    constructor(props) {
        super(props);
    this.state = {
        data : [],
    ...
    findGameTypeById = id => {
        ...
        if (gameTypes && gameTypes.data.length) {
            if (id) {
                ...
                return  gameTypes.data.filter(data => data.id === id)[1]
            }

            return gameTypes.data[1]
        }
...
const mapStateToProps = ({ auth, gamificationState }) => {
    const { authUser } = auth;
    const { gameTypes, createGame, updateGame, loader, alertMessage, showMessage, createSuccess, createFailed, data } = gamificationState;

    return { authUser, gameTypes, createGame, updateGame, loader, alertMessage, showMessage, createSuccess, createFailed, data };
};

const WrappedCreateUpdateForm = Form.create({ name: 'createUpdateGamification' })(GamificationCreateUpdateSurvey);
