...
import {
    getGameTypes, createGamificationQuiz, createGamificationSurvey, createGamificationSpinner, viewGamification, resetStatus
...
class GamificationCreateUpdateSpinner extends Component {
    constructor(props) {
        super(props);
    ...
    componentDidUpdate(prevProps, prevState, snapshot) {
        if (prevProps.updateSuccess && !prevProps.updateFailed){
            this.setState({
                msgContent : 'Updated Successfully',
                msgShow : true,
                msgType : 'success'
            })
        }else if (!prevProps.updateSuccess && prevProps.updateFailed){
            this.setState({
        ...
        if (prevProps.createSuccess && !prevProps.createFailed){
            this.setState({
                msgContent : 'Created Successfully',
                msgShow : true,
                msgType : 'success'
            })
        }else if (!prevProps.createSuccess && prevProps.createFailed){
            this.setState({
    ...
    render() {
        ...
        let {data, msgShow, msgType, msgContent,} = this.state;
            ...
            <div>
                ...
                >
                    <Card>
                        ...
                        {/*    {getFieldDecorator('gameType',{*/}
                        {/*        initialValue: data.gamificationType,*/}
                        {/*        rules: [{*/}
                        {/*            required: true,*/}
...
const mapStateToProps = ({ auth, gamificationState }) => {
    const { authUser } = auth;
    const { gameTypes, createGame, updateGame, loader, alertMessage, showMessage, createSuccess, createFailed, } = gamificationState;

    return { authUser, gameTypes, createGame, updateGame, loader, alertMessage, showMessage, createSuccess, createFailed, };
};

const WrappedCreateUpdateForm = Form.create({ name: 'createUpdateGamification' })(GamificationCreateUpdateSpinner);
