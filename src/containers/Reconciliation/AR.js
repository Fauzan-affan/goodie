...
class SearchReconciliationReceiveble extends Component {
    ...
    fetchData = (filterAndSort, isDownload)=>{
        ...
        const {pagination, sorter, search, period, startDate, endDate,
            filters, periodWeek, periodMonth, periodYear
        } = filterAndSort;
        credential.page = 0;
        credential.sortBy = 0;
        credential.sort = 1;
            ...
            if(filters.pointTransactionType!=null){
                if(!filters.pointTransactionType.isEmpty){
                    filters.pointTransactionType.forEach(item=>{
                        credential.pointTransactionTypeList += item+',test';
                    });
                    credential.pointTransactionTypeList = credential.pointTransactionTypeList;
                }
            }
        }
    ...
    render() {
        ...
        let columns = [
            ...
            },
            {
                ...
                label : 'Merchant Name',
                key: 'merchantPayable',
                filteredValue: filteredInfo.merchantPayable || null,
                // sorter: (a, b) => a.name.length - b.name.length,
                // sortOrder: sortedInfo.columnKey === 'period' && sortedInfo.order
            },
            {
...
export default connect(mapStateToProps, {
    searchReconciliationPayable,
    // viewReconciliation,
    filterSortSearch, clearFilterSortSearch, resetStatus})(SearchReconciliationReceiveble);
