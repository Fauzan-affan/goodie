    ...
    createContent = (formContents) => {
        ...
        formContents.forEach((formContent, i) => {
            ...
            if(formContent.type === 'text' ){
                ...
                comp.push(
                    <FormItem {...formItemLayout} label={formContent.label} key={i}>
                        <span className='ant-form-text'>
                            <img src={formContent.value} alt='image not found' width={'300px'}/>
                        </span>
                    </FormItem>
                );
                ...
                if(formContent.listData != null){
                    comp.push(
                        <FormItem {...formListLayout} label={formContent.label} key={i}>
                            <Table className="gx-table-responsive" pagination = {true} columns={formContent.columns} dataSource={formContent.listData}/>
                        </FormItem>
                    );
                }
                ...
                comp.push(
                    <FormItem {...formItemLayout} label={formContent.label} key={i}>
                        <div className='ant-form-text' dangerouslySetInnerHTML={{ __html: formContent.value }}></div>
                    </FormItem>
                );
